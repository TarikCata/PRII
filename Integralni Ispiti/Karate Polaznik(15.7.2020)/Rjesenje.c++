...
public:
    ...
    void AddElement(const T1& el1, const T2& el2,int lokacija) {
        if (lokacija == max || lokacija < 0)
        ...
        _elementi2[lokacija] = new T2(el1);
        _trenutno++;
...
public:
    ...
    bool AddOcjena(int br, Datum& d) {
        for (size_t i = 0; i < _ocjene->getTrenutno(); i++)
            if (d - _ocjene->getElement2(i) < 3)
                return false;
        _ocjene->AddElement(br, d);
        return true;
    }
    bool operator==(const Tehnika& t) { 
        if (t._ocjene->getTrenutno() == _ocjene->getTrenutno()) {
            for (size_t i = 0; i < _ocjene->getTrenutno(); i++) {
                if (_ocjene[i].getElement1(i) == t._ocjene[i].getElement1(i))
                    if (_ocjene[i].getElement2(i) == t._ocjene[i].getElement2(i))
                        if (strcmp(t._naziv, _naziv) == 0)
                            return true;
            }
        }
        return false;
    ...
    float Prosje()const {
        if (_ocjene->getTrenutno() == 0)return 0.0f;
        float sum = 0.0f;
        for (size_t i = 0; i < _ocjene->getTrenutno(); i++)
    ...
    friend ostream& operator<<(ostream& cout, const Tehnika& t) {
        ...
        for (size_t i = 0; i < t._ocjene->getTrenutno(); i++)
            cout << "Ocjene:" << t._ocjene->getElement1(i) << "\tDatum:" << t._ocjene->getElement2(i) << "\n";
        cout << "Prosjek:" << t.Prosje();
        return cout;
    }
...
public:
    ...
    float GetProsjek() {
        ...
        float suma = 0.0f;
        for (size_t i = 0; i < _polozeneTehnike.size(); i++)
            suma += _polozeneTehnike[i]->Prosje();
        return float(suma /= _polozeneTehnike.size());
    }
};
...
public:
    ...
    virtual void Info()const { cout << endl; }
};
...
class KaratePolaznik  : public Korisnik{
    ...
    void PosaljiEmail(const char* nazivTeknike, Pojas p, float prosjekNaPojasu) {
        m.lock();
        cout << crt << "FROM:info@karate.ba\nTO:" << _emailAdresa << "\nPostovani,\n" << _imePrezime << " evidentirana vam je tehnika " <<
            nazivTeknike << "za " << p << ".Dosadasnji uspjeh (prosjek ocjena) na pojasu " << nazivTeknike << " iznosi " << prosjekNaPojasu <<
            "a ukupni uspjeh (prosjek ocjena) na svim pojasevima iznosi " << GetUkupniProsjek() << "\nPozdrav\n\nKARATE Team.\n";
        m.unlock();
    }
public:
    ...
    float GetUkupniProsjek() {
        ...
        return float(suma /= _polozeniPojasevi.size());
    }
    bool AddTehniku(Pojas p,Tehnika& t) {
        for (size_t i = 0; i < _polozeniPojasevi.size(); i++)
            if (_polozeniPojasevi[i].GetPojas() == p) {
                if (_polozeniPojasevi[i].AddTehniku(t)) {
                    thread td(&KaratePolaznik::PosaljiEmail, this, t.GetNaziv(), p, _polozeniPojasevi[i].GetProsjek());
                    td.join();  
                    return true;
                }
                return false;
...
const char* GetOdgovorNaPrvoPitanje() {
    ...
    return "Odgovor -> Konverziju pokazivača na objekte klasa koje nemaju nikakvih međusobnih relacija je moguće izvršiti korištenjem reinterpret_cast operatora";
}
const char* GetOdgovorNaDrugoPitanje() {
    cout << "Pitanje -> Ukratko opisite znacaj i vrste pametnih pokazivaca.\n";
    return "Odgovor -> korištenjem pametnih pokazivača, ne moramo ručno dealocirati zauzetu memoriju već se ona automatski briše nakon što više nije potrebna";
}
void main() {
